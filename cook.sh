...
apt install nginx
nano /etc/nginx/sites-available/pinkiehoney.eu.org
ln -s /etc/nginx/sites-available/pinkiehoney.eu.org /etc/nginx/sites-enabled/pinkiehoney.eu.org
nginx -t
systemctl restart nginx
sudo -u thelounge thelounge add tiffany
sudo -u thelounge thelounge install thelounge-theme-mininapse
apt install certbot python3-certbot-nginx
certbot --nginx -d pinkiehoney.eu.org
